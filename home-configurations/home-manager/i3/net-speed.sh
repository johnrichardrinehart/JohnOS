...
external_ip=""
get_external_ip() {
  external_ip=$(wget -q -O - https://wgetip.com/)
}

get_external_ip

i3status | (read -r line && echo "$line" && read -r line && echo "$line" && read -r line && echo "$line" && update_rate && while :; do
  read -r line
  update_rate
  echo ",[{\"full_text\":\"${rate}\"},{\"full_text\":\"E: ${external_ip}\"}, ${line#,\[}" || exit 1
done)
