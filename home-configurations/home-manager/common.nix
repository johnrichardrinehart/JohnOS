...
in
{
  ...
  config = {
    ...
    // lib.optionalAttrs
      (
        (lib.attrByPath [ "rock5c" "sessions" "hyprland" "enable" ] false osConfig)
        || (lib.attrByPath [ "dev" "johnrinehart" "desktop" "wl-hyprland" "enable" ] false osConfig)
      ) {
      # Hyprland prefers ~/.config/hypr/hyprland.conf over /etc/xdg/hypr/hyprland.conf.
      # Manage a tiny user config here so stale autogenerated files do not mask the
      # system session config installed by the NixOS Hyprland module.
    ...
    dconf.settings = lib.mkIf
      (!(lib.attrByPath [ "rock5c" "media" "kodi" "enable" ] false osConfig
        && lib.attrByPath [ "rock5c" "media" "kodi" "variant" ] "auto" osConfig == "gbm")) {
