  ...
  config = lib.mkIf cfg.enable {
    ...
    fileSystems = builtins.listToAttrs (builtins.map (x: {
      ...
      value = {
        ...
        options = [ "_netdev" "x-systemd.automount" "allow_other" "use_path_request_style" "url=${x.url}" "passwd_file=${x.passwordFile}" "connect_timeout=30" "retries=90" "dbglevel=debug" "curldbg=body" ];
      };
    }) cfg.mounts);
  };
}
