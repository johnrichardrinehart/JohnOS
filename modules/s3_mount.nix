...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    fileSystems = builtins.listToAttrs (builtins.map (x: {
      ...
      value = {
        device = x.bucketName;
        mountPoint = "/mnt/${x.bucketName}";
        fsType = "fuse.${builtins.unsafeDiscardStringContext pkgs.s3fs}/bin/s3fs";
        noCheck = true;
        # cf. https://unix.stackexchange.com/a/349278
        options = [ "_netdev" "x-systemd.after=network-online.target" "allow_other" "use_path_request_style" "url=${x.url}" "passwd_file=${x.passwordFile}" "connect_timeout=30" "retries=30" ];
      };
    }) cfg.mounts);
  };
}
