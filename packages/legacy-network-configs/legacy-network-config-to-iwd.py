...
from wifi_profile_convert import IWD_DESTINATION, convert_legacy_profiles


def main():
    parser = argparse.ArgumentParser(
        description="Import supported legacy Wi-Fi configs into iwd.",
    )
    parser.add_argument(
        "--destination",
        type=type(IWD_DESTINATION),
        default=IWD_DESTINATION,
        help=f"iwd state directory, default: {IWD_DESTINATION}",
    )
    parser.add_argument(
        "--source",
        action="append",
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="replace existing iwd profiles",
    )
    parser.add_argument(
        "--show-skipped",
        action="store_true",
